...
async function seedDatabase() {
    ...
    try {
        ...
        for (const d of DISTRICTS) {
            const res = await (0, database_service_1.query)(`INSERT INTO districts (name, code, latitude, longitude)
         VALUES ($1, $2, $3, $4)
         ON CONFLICT (name) DO UPDATE SET updated_at = NOW()
         RETURNING id`, [d.name, d.code, d.lat, d.lng]);
            districtIds.push(res.rows[0].id);
        }
        logger_1.logger.info(`Seeded ${districtIds.length} Districts`);
        ...
        for (let i = 0; i < STATIONS.length; i++) {
         ...
         RETURNING id`, [STATIONS[i], `PS-${100 + i}`, distId, `080-2294${2000 + i}`]);
            stationIds.push(res.rows[0].id);
        }
        logger_1.logger.info(`Seeded ${stationIds.length} Police Stations`);
        // 3. Seed Users
       ...
       RETURNING id`, [passwordHash, districtIds[0], stationIds[0]]);
        ...
        logger_1.logger.info('Seeded 200 Criminal Profiles successfully');
        ...
        logger_1.logger.info('Seeded 1000 FIR Records successfully');
        // 6. Seed 500 Cases
        ...
        for (let i = 1; i <= 500; i++) {
            ...
            await (0, database_service_1.query)(`INSERT INTO cases (case_number, title, description, crime_category, status, priority, district_id, station_id, assigned_officer_id, ai_risk_score)
        ...
        logger_1.logger.info('Seeded 500 Case Files successfully');
